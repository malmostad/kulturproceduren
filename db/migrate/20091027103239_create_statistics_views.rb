class CreateStatisticsViews < ActiveRecord::Migration
  def self.up
	...
	sql =  "CREATE OR REPLACE VIEW num_booked AS
		select t.group_id as group_id , e.name as event_name, count(t.id) as num_booked
		from tickets t 
   		  inner join events e on t.event_id = e.id
		where t.state in ( 2,3 ) 
		group by group_id,e.name
		order by group_id, e.name"
	execute sql
	sql =  "CREATE OR REPLACE VIEW num_adult AS
		select t.group_id as group_id , e.name as event_name, count(t.id) as num_adult
		from tickets t 
		   inner join events e on t.event_id = e.id
	...
	sql =  "CREATE OR REPLACE VIEW num_children AS
		select t.group_id as group_id , e.name as event_name, count(t.id) as num_children
		from tickets t 
		   inner join events e on t.event_id = e.id
	...
	sql =  "CREATE OR REPLACE VIEW statistics AS
		SELECT ag.district_name , ag.school_name , ag.group_name , nb.event_name , nb.num_booked , nc.num_children , na.num_adult
		from all_groups ag 
